...
// at six widths, in a light galaxy and a dark one, in English and in Arabic,
// and in the day-one empty state — which is the state most new learners see and
// the only one where every badge is locked and every ring is empty.
//
...
/*
  One dark galaxy and one light one, because the score ring, the badge medals and
  the four tile washes all flip convention between the two — and Arabic, because
  the ring must NOT mirror while the row of tiles must.
*/
const VARIANTS = [
    { id: 'sql-dark', query: 'tab=sql&theme=andromeda&lang=en' },
    { id: 'linux-dark', query: 'tab=linux&theme=andromeda&lang=en' },
    { id: 'python-dark', query: 'tab=python&theme=andromeda&lang=en' },
    { id: 'sql-light', query: 'tab=sql&theme=cartwheel&lang=en' },
    { id: 'linux-light', query: 'tab=linux&theme=cartwheel&lang=en' },
    { id: 'sql-ar', query: 'tab=sql&theme=andromeda&lang=ar' },
    { id: 'noaccess', query: 'state=noaccess&theme=cartwheel&lang=en' },
];
...
for (const width of WIDTHS) {
    /* Every variant at the two widths a reader is most likely to be on, and one
       ...
       palette, but the palette is the whole point at 1440 and 390. */
    const variants = width === 1440 || width === 390 ? VARIANTS : [VARIANTS[0], VARIANTS[1]];
