...
const FAMILY_LABELS: Record<string, string> = {
    ...
    // The AI engineering platform, agent orchestration, and the management
    // simulation. Three families rather than one `ai` family, because a lab
    // about a learning curve, a lab about an agent's step budget and a lab
    // about a one-to-one cadence share nothing at all.
...
export const GUI_PANELS: Record<string, GuiPanel[]> = {
    ...
    azure: [
        ...
        /* ── the platform layer ───────────────────────────────────────── */
        { id: 'platform', title: 'Platform', kind: 'stats', path: 'platform',
            tool: 'azure_network_gui' },
        {
            id: 'vnets', title: 'Virtual networks', kind: 'table',
            path: 'vnets', tool: 'azure_network_gui',
            empty: 'No virtual networks',
            columns: [col('name', 'Name'), col('cidr', 'Address space', 'code'),
                col('group', 'Resource group'), col('subnets', 'Subnets', 'number'),
                col('peers', 'Peered to', 'list'), col('dns', 'DNS'),
                col('status', 'Status', 'badge')],
        },
        {
            id: 'subnets', title: 'Subnets', kind: 'table', path: 'subnets',
            tool: 'azure_network_gui', empty: 'No subnets',
            // `nsg` and `route_table` are columns of their own and deliberately
            // EMPTY when absent: a subnet with no NSG has only the platform
            // defaults, which allow all VNet-to-VNet traffic, and that is the
            // fact the panel exists to make visible.
            columns: [col('vnet', 'VNet'), col('name', 'Subnet'),
                col('prefix', 'Prefix', 'code'), col('nsg', 'NSG'),
                col('route_table', 'Route table'),
                col('delegation', 'Delegated to'),
                col('endpoints', 'Service endpoints', 'list'),
                col('status', 'Status', 'badge')],
        },
        {
            id: 'nsg_rules', title: 'NSG rules', kind: 'table',
            path: 'nsg_rules', tool: 'azure_network_gui',
            empty: 'No rules of your own - only the platform defaults',
            columns: [col('nsg', 'NSG'), col('priority', 'Priority', 'number'),
                col('name', 'Rule'), col('direction', 'Direction'),
                col('access', 'Access', 'badge'), col('protocol', 'Protocol'),
                col('source', 'Source', 'code'),
                col('destination', 'Destination', 'code'),
                col('ports', 'Ports', 'code')],
        },
        {
            id: 'routes', title: 'Routes', kind: 'table', path: 'routes',
            tool: 'azure_network_gui', empty: 'No user-defined routes',
            columns: [col('table', 'Route table'), col('name', 'Route'),
                col('prefix', 'Prefix', 'code'),
                col('next_hop', 'Next hop', 'badge'),
                col('next_hop_ip', 'Next hop IP', 'code'),
                col('attached', 'Attached to', 'list')],
        },
        {
            id: 'peerings', title: 'Peerings', kind: 'table', path: 'peerings',
            tool: 'azure_network_gui', empty: 'No peerings',
            // `state` is the column that matters: a peering is TWO objects and
            // one alone leaves it Initiated, which reads as a timeout.
            columns: [col('vnet', 'From'), col('remote', 'To'),
                col('state', 'State', 'badge'),
                col('forwarded', 'Forwarded traffic', 'badge'),
                col('gateway_transit', 'Gateway transit', 'badge'),
                col('remote_gateways', 'Use remote gateways', 'badge')],
        },
        {
            id: 'private_endpoints', title: 'Private endpoints', kind: 'table',
            path: 'private_endpoints', tool: 'azure_network_gui',
            empty: 'No private endpoints',
            // `status` here is the whole lesson: an endpoint with no zone
            // group, or a zone that is not linked to the consuming VNet,
            // leaves the name resolving PUBLICLY - and everything works.
            columns: [col('name', 'Name'), col('target', 'Target'),
                col('group_id', 'Group id'), col('ip', 'Private IP', 'code'),
                col('fqdn', 'FQDN', 'code'), col('zone', 'Zone', 'code'),
                col('zone_linked', 'Zone linked', 'badge'),
                col('connection', 'Connection', 'badge'),
                col('status', 'Resolves', 'badge')],
        },
        {
            id: 'dns_zones', title: 'Private DNS zones', kind: 'table',
            path: 'dns_zones', tool: 'azure_network_gui',
            empty: 'No private DNS zones',
            columns: [col('name', 'Zone', 'code'), col('group', 'Resource group'),
                col('links', 'Linked networks', 'list'),
                col('records', 'Records', 'number'),
                col('status', 'Status', 'badge')],
        },
        {
            id: 'firewalls', title: 'Firewalls', kind: 'table',
            path: 'firewalls', tool: 'azure_network_gui', empty: 'No firewall',
            columns: [col('name', 'Name'), col('vnet', 'VNet'),
                col('sku', 'SKU'), col('private_ip', 'Private IP', 'code'),
                col('network_rules', 'Network rules', 'number'),
                col('application_rules', 'Application rules', 'number'),
                col('threat_intel', 'Threat intel')],
        },
        {
            id: 'firewall_rules', title: 'Firewall rules', kind: 'table',
            path: 'firewall_rules', tool: 'azure_network_gui',
            empty: 'No firewall rules - which means an implicit deny',
            // NETWORK rules are evaluated before APPLICATION rules, so the
            // `kind` column is what explains an FQDN restriction that never
            // fires.
            columns: [col('firewall', 'Firewall'), col('kind', 'Kind', 'badge'),
                col('collection', 'Collection'), col('name', 'Rule'),
                col('action', 'Action', 'badge'), col('protocol', 'Protocol'),
                col('source', 'Source', 'code'), col('target', 'Target', 'code'),
                col('ports', 'Ports', 'code')],
        },
        {
            id: 'path_tests', title: 'Path tests', kind: 'table',
            path: 'path_tests', tool: 'azure_network_gui',
            empty: 'No path tests yet',
            columns: [col('source', 'From', 'code'),
                col('destination', 'To', 'code'), col('port', 'Port', 'number'),
                col('protocol', 'Protocol'), col('hops', 'Hops', 'number'),
                col('status', 'Result', 'badge'), col('detail', 'Detail')],
        },
        {
            id: 'hierarchy', title: 'Management groups', kind: 'table',
            path: 'hierarchy', tool: 'azure_governance_gui',
            empty: 'No management groups',
            columns: [col('name', 'Name', 'code'), col('display', 'Display name'),
                col('parent', 'Parent'),
                col('subscriptions', 'Subscriptions', 'number'),
                col('policies', 'Policies', 'number'),
                col('status', 'Kind', 'badge')],
        },
        {
            id: 'policy_assignments', title: 'Policy assignments',
            kind: 'table', path: 'policy_assignments',
            tool: 'azure_governance_gui',
            empty: 'No policy - so anything is allowed',
            columns: [col('name', 'Assignment'), col('display', 'What it checks'),
                col('effect', 'Effect', 'badge'), col('scope', 'Scope', 'code'),
                col('identity', 'Identity', 'badge'),
                col('evaluated', 'Evaluated', 'number'),
                col('non_compliant', 'Non-compliant', 'number'),
                col('compliance', 'Rate'), col('status', 'Status', 'badge')],
        },
        {
            id: 'compliance', title: 'Compliance', kind: 'table',
            path: 'compliance', tool: 'azure_governance_gui',
            empty: 'Nothing evaluated yet',
            columns: [col('resource', 'Resource'), col('kind', 'Kind'),
                col('assignment', 'Assignment'), col('effect', 'Effect', 'badge'),
                col('scope', 'Scope', 'code'), col('status', 'State', 'badge'),
                col('reason', 'Why')],
        },
        {
            id: 'role_assignments', title: 'Role assignments', kind: 'table',
            path: 'role_assignments', tool: 'azure_governance_gui',
            empty: 'No role assignments',
            columns: [col('principal', 'Principal'), col('kind', 'Type', 'badge'),
                col('role', 'Role'), col('scope', 'Scope', 'code'),
                col('status', 'Breadth', 'badge'), col('note', 'What it grants')],
        },
        {
            id: 'principals', title: 'Principals', kind: 'table',
            path: 'principals', tool: 'azure_governance_gui',
            empty: 'No principals',
            columns: [col('name', 'Name'), col('kind', 'Type', 'badge'),
                col('id', 'Object id', 'code'),
                col('assignments', 'Role assignments', 'number')],
        },
        {
            id: 'workspaces', title: 'Workspaces', kind: 'table',
            path: 'workspaces', tool: 'azure_monitor_gui',
            empty: 'No Log Analytics workspace',
            columns: [col('name', 'Name'), col('group', 'Resource group'),
                col('sku', 'SKU'), col('retention', 'Retention days', 'number'),
                col('tables', 'Tables with data', 'number')],
        },
        {
            id: 'diagnostics', title: 'Diagnostic settings', kind: 'table',
            path: 'diagnostics', tool: 'azure_monitor_gui',
            empty: 'Nothing is sending logs',
            // A resource with no setting is SILENT however many queries you
            // write, which is why this panel lists every resource rather than
            // only the ones that are wired up.
            columns: [col('resource', 'Resource'), col('kind', 'Kind'),
                col('workspace', 'Workspace'), col('by', 'Created by'),
                col('status', 'Status', 'badge')],
        },
        {
            id: 'alerts', title: 'Alert rules', kind: 'table', path: 'alerts',
            tool: 'azure_monitor_gui', empty: 'No alert rules',
            columns: [col('name', 'Name'), col('condition', 'Condition', 'code'),
                col('window', 'Window'), col('frequency', 'Frequency'),
                col('severity', 'Severity', 'number'),
                col('action', 'Action group'), col('status', 'Status', 'badge')],
        },
        {
            id: 'queries', title: 'Queries you have run', kind: 'table',
            path: 'queries', tool: 'azure_monitor_gui', empty: 'No queries yet',
            columns: [col('query', 'KQL', 'code'), col('rows', 'Rows', 'number'),
                col('at', 'When')],
        },
        {
            id: 'cost', title: 'Monthly cost', kind: 'table', path: 'cost',
            tool: 'azure_cost_gui', empty: 'Nothing is costing anything yet',
            columns: [col('resource', 'Resource'), col('kind', 'Kind'),
                col('sku', 'SKU'), col('group', 'Resource group'),
                col('monthly', 'Per month', 'number'), col('share', 'Share'),
                col('owner', 'Owner tag'), col('status', 'Tagged', 'badge')],
        },
        {
            id: 'budgets', title: 'Budgets', kind: 'table', path: 'budgets',
            tool: 'azure_cost_gui', empty: 'No budget - so nobody is watching',
            columns: [col('name', 'Name'), col('amount', 'Amount', 'number'),
                col('run_rate', 'Run rate', 'number'), col('used', 'Used'),
                col('thresholds', 'Alert at'), col('action', 'Action'),
                col('status', 'Status', 'badge')],
        },
        {
            id: 'deployments', title: 'Deployments', kind: 'table',
            path: 'deployments', tool: 'azure_iac_gui', empty: 'No deployments',
            columns: [col('name', 'Name'), col('scope', 'Scope', 'badge'),
                col('group', 'Resource group'), col('template', 'Template', 'code'),
                col('created', 'Created', 'number'),
                col('updated', 'Updated', 'number'),
                col('status', 'Status', 'badge')],
        },
        {
            id: 'whatif', title: 'Last what-if', kind: 'table', path: 'whatif',
            tool: 'azure_iac_gui', empty: 'No what-if run yet',
            columns: [col('file', 'Template', 'code'),
                col('change', 'Change', 'badge'), col('name', 'Resource'),
                col('type', 'Type', 'code'), col('detail', 'What changes')],
        },
    ],
    terraform: [
    ...
    */
    ...
    mlops: [
        { id: 'stats', title: 'Platform', kind: 'stats', path: 'stats',
            tool: 'mlops_gui' },
        {
            id: 'datasets', title: 'Datasets', kind: 'table', path: 'datasets',
            tool: 'mlops_gui', empty: 'No datasets',
            // `leaks` is the column the whole pane is for: a model trained on
            // a leaked split scores higher than it deserves to, and the model
            // is not better - the number is wrong.
            columns: [col('name', 'Dataset'), col('rows', 'Rows', 'number'),
                col('target', 'Target'), col('group', 'Natural grouping'),
                col('positive_rate', 'Positive'), col('method', 'Split'),
                col('train', 'Train', 'number'), col('val', 'Val', 'number'),
                col('test', 'Test', 'number'),
                col('features', 'Features', 'list'),
                col('status', 'Status', 'badge'), col('leaks', 'Leakage')],
        },
        {
            id: 'pipeline', title: 'Pipeline', kind: 'table', path: 'pipeline',
            tool: 'mlops_gui', empty: 'No pipeline stages',
            columns: [col('stage', 'Stage'), col('cmd', 'Command', 'code'),
                col('deps', 'Depends on', 'code'), col('outs', 'Outputs', 'code'),
                col('runs', 'Runs', 'number'), col('status', 'Status', 'badge')],
        },
        {
            id: 'runs', title: 'Runs', kind: 'table', path: 'runs',
            tool: 'mlops_gui', empty: 'No training runs yet',
            columns: [col('id', 'Run', 'code'), col('experiment', 'Experiment'),
                col('model', 'Base model'), col('method', 'Method', 'badge'),
                col('epochs', 'Epochs', 'number'),
                col('best_epoch', 'Best epoch', 'number'),
                col('val_loss', 'Val loss', 'number'),
                col('val_metric', 'Val metric', 'number'),
                col('memory_gb', 'VRAM GiB', 'number'),
                col('leak', 'Leak factor', 'number'),
                col('status', 'Status', 'badge')],
        },
        {
            id: 'curves', title: 'Learning curves', kind: 'table',
            path: 'curves', tool: 'mlops_gui', empty: 'Nothing trained yet',
            columns: [col('run', 'Run', 'code'), col('epoch', 'Epoch', 'number'),
                col('train_loss', 'Train loss', 'number'),
                col('val_loss', 'Val loss', 'number'),
                col('val_metric', 'Val metric', 'number'),
                col('status', 'Note', 'badge')],
        },
        {
            id: 'registry', title: 'Model registry', kind: 'table',
            path: 'registry', tool: 'mlops_gui', empty: 'Nothing registered',
            columns: [col('name', 'Model'), col('version', 'Version', 'number'),
                col('stage', 'Stage', 'badge'), col('run', 'Run', 'code'),
                col('base', 'Base'), col('format', 'Format'),
                col('size_mb', 'Size MB', 'number'),
                col('val_metric', 'Val metric', 'number')],
        },
        {
            id: 'evals', title: 'Evaluations', kind: 'table', path: 'evals',
            tool: 'mleval_gui', empty: 'No evaluations',
            // The four cells are columns of their own, because precision and
            // recall are the ratios OVER them and a student has to be able to
            // check the arithmetic.
            columns: [col('model', 'Model'), col('suite', 'Suite', 'badge'),
                col('rows', 'Rows', 'number'),
                col('threshold', 'Threshold', 'number'),
                col('accuracy', 'Accuracy', 'number'),
                col('precision', 'Precision', 'number'),
                col('recall', 'Recall', 'number'), col('f1', 'F1', 'number'),
                col('balanced', 'Balanced', 'number'),
                col('tp', 'TP', 'number'), col('fp', 'FP', 'number'),
                col('fn', 'FN', 'number'), col('tn', 'TN', 'number')],
        },
        {
            id: 'slices', title: 'Per slice', kind: 'table', path: 'slices',
            tool: 'mleval_gui', empty: 'Run an eval with --slices',
            columns: [col('model', 'Model'), col('slice', 'Slice', 'code'),
                col('rows', 'Rows', 'number'),
                col('accuracy', 'Accuracy', 'number'),
                col('precision', 'Precision', 'number'),
                col('recall', 'Recall', 'number'), col('f1', 'F1', 'number'),
        ...
        {
            id: 'traces', title: 'Knowledge tracing', kind: 'table',
            path: 'traces', tool: 'mleval_gui', empty: 'No traces',
            columns: [col('trace', 'Trace'), col('n', 'N', 'number'),
                col('answer', 'Answer', 'badge'),
                col('predicted', 'P(correct)', 'number'),
                col('prior', 'Prior', 'number'),
                col('posterior', 'Posterior', 'number'),
                col('mastery', 'Mastery', 'number')],
        },
        {
            id: 'recsys', title: 'Recommenders', kind: 'table', path: 'recsys',
            tool: 'mleval_gui', empty: 'No recommender runs',
            columns: [col('method', 'Method', 'badge'), col('k', 'k', 'number'),
                col('precision_at_k', 'precision@k', 'number'),
                col('recall_at_k', 'recall@k', 'number'),
                col('ndcg_at_k', 'NDCG@k', 'number'),
                col('catalogue_coverage', 'Coverage', 'number'),
                col('cold_start', 'Cold start')],
        },
        {
            id: 'serving', title: 'Serving', kind: 'table', path: 'serving',
            tool: 'mlserve_gui', empty: 'Nothing is being served',
            // Weights and KV cache are separate columns because the KV cache
            // is the part that surprises people: it scales with context AND
            // with concurrency, and it is usually the larger of the two.
            columns: [col('name', 'Name'), col('base', 'Base'),
                col('runtime', 'Runtime', 'badge'), col('dtype', 'Precision'),
                col('batch', 'Batch', 'number'),
                col('max_len', 'Max length', 'number'),
                col('weights_gb', 'Weights GiB', 'number'),
                col('kv_gb', 'KV cache GiB', 'number'),
                col('total_gb', 'Total GiB', 'number'),
                col('p50_ms', 'p50 ms', 'number'),
                col('p95_ms', 'p95 ms', 'number'),
                col('tokens_per_s', 'Tokens/s', 'number'),
                col('status', 'Status', 'badge')],
        },
        {
            id: 'monitors', title: 'Drift', kind: 'table', path: 'monitors',
            tool: 'mlserve_gui', empty: 'No drift monitors',
            columns: [col('model', 'Model'), col('feature', 'Feature'),
                col('psi', 'PSI', 'number'), col('verdict', 'Verdict'),
                col('baseline', 'At training', 'number'),
                col('current', 'Now', 'number'), col('window', 'Window'),
                col('status', 'Status', 'badge')],
        },
        {
            id: 'abtests', title: 'A/B tests', kind: 'table', path: 'abtests',
            tool: 'mlserve_gui', empty: 'No A/B tests',
            // `needed` is sized from the MDE declared up front, never from the
            // effect seen so far - sizing a test from its own interim result
            // is the arithmetic that makes peeking look rigorous.
            columns: [col('id', 'Test'), col('metric', 'Metric'),
                col('days', 'Days', 'number'), col('control', 'Control'),
                col('variant', 'Variant'), col('rate_a', 'Control rate'),
                col('rate_b', 'Variant rate'), col('lift', 'Lift'),
                col('p_value', 'p', 'number'), col('ci', 'CI'),
                col('mde', 'Declared MDE'),
                col('needed', 'Needed per arm', 'number'),
                col('status', 'Verdict', 'badge'),
                // EVERY VERDICT THIS TEST HAS REPORTED, not just the current
                // one. The lesson of the A/B lab is the comparison between an
                // interim call and a final one on the SAME test, and `status`
                // is overwritten by each report - so without this column the
                // thing the lab is about is the one thing the panel cannot
                // show.
                col('reported', 'Reported so far')],
        },
        {
            id: 'exports', title: 'Exports', kind: 'table', path: 'exports',
            tool: 'mlserve_gui', empty: 'Nothing exported',
            columns: [col('model', 'Model'), col('base', 'Base'),
                col('format', 'Format', 'badge'), col('opset', 'Opset', 'number'),
                col('size_mb', 'Size MB', 'number')],
        },
        {
            id: 'models', title: 'Model catalogue', kind: 'table',
            path: 'models', tool: 'mlserve_gui', empty: 'No models',
            columns: [col('name', 'Model', 'code'), col('params', 'Parameters'),
                col('kind', 'Architecture', 'badge'),
                col('hidden', 'Hidden', 'number'),
                col('layers', 'Layers', 'number'),
                col('context', 'Context', 'number'),
                col('local', 'Downloaded', 'badge')],
        },
    ],
    agentlab: [
        { id: 'stats', title: 'Agent', kind: 'stats', path: 'stats',
            tool: 'agent_gui' },
        {
            id: 'runs', title: 'Runs', kind: 'table', path: 'runs',
            tool: 'agent_gui', empty: 'Nothing has run yet',
            // `stopped_by` is the column that matters. "It did not finish" is
            // not a diagnosis; "it hit the step budget with two tools left in
            // its plan" is.
            columns: [col('id', 'Run'), col('task', 'Task'),
                col('planner', 'Planner', 'badge'), col('model', 'Model'),
                col('steps', 'Steps', 'number'),
                col('stopped_by', 'Stopped by', 'badge'),
                col('goal', 'Goal reached', 'badge'),
                col('tokens', 'Tokens', 'number'), col('cost', 'Cost', 'number'),
                col('seconds', 'Seconds', 'number'),
                col('peak_window', 'Peak window', 'number'),
                col('repeats', 'Repeats', 'number'),
                col('refusals', 'Refusals', 'number'),
                col('compactions', 'Compactions', 'number'),
                col('overflows', 'Overflows', 'number'),
                col('constraint', 'Constraint held', 'badge'),
                col('grade', 'Grade', 'number')],
        },
        {
            id: 'steps', title: 'Steps', kind: 'table', path: 'steps',
            tool: 'agent_gui', empty: 'No steps',
            columns: [col('run', 'Run'), col('n', '#', 'number'),
                col('tool', 'Tool', 'code'), col('args', 'Arguments', 'code'),
                col('decision', 'Decision', 'badge'), col('reason', 'Why'),
                col('obs_tokens', 'Observation', 'number'),
                col('window', 'Window', 'number'), col('note', 'Note')],
        },
        {
            id: 'checks', title: 'Trajectory checks', kind: 'table',
            path: 'checks', tool: 'agent_gui', empty: 'Nothing graded yet',
            columns: [col('run', 'Run'), col('check', 'Check'),
                col('status', 'Result', 'badge'), col('detail', 'Detail')],
        },
        {
            id: 'tools', title: 'Tools', kind: 'table', path: 'tools',
            tool: 'agent_gui', empty: 'No tools',
            columns: [col('tool', 'Tool', 'code'), col('args', 'Arguments'),
                col('destructive', 'Destructive', 'badge'),
                col('obs_tokens', 'Observation tokens', 'number'),
                col('latency_ms', 'Latency ms', 'number'),
                col('given', 'Given', 'badge'), col('status', 'Policy', 'badge'),
                col('summary', 'What it does')],
        },
        {
            id: 'policy', title: 'Guardrails', kind: 'table', path: 'policy',
            tool: 'agent_gui', empty: 'No policy',
            columns: [col('rule', 'Rule'), col('value', 'Value', 'code')],
        },
        {
            id: 'memory', title: 'Memory', kind: 'table', path: 'memory',
            tool: 'agent_gui', empty: 'Nothing remembered',
            columns: [col('tier', 'Tier', 'badge'),
                col('entries', 'Entries', 'number'),
                col('survives', 'Survives the run', 'badge'),
                col('text', 'Most recent')],
        },
        {
            id: 'approvals', title: 'Approvals', kind: 'table',
            path: 'approvals', tool: 'agent_gui', empty: 'None granted',
            columns: [col('tool', 'Tool', 'code'),
                col('args', 'Arguments', 'code'), col('used', 'Used', 'badge'),
                col('at', 'When')],
        },
        {
            id: 'planners', title: 'Planners', kind: 'table', path: 'planners',
            tool: 'agent_gui', empty: 'No planners',
            columns: [col('planner', 'Planner', 'code'), col('label', 'What it is'),
                col('overhead', 'Token overhead', 'number'),
    ...
    manage: [
        { id: 'stats', title: 'This sprint', kind: 'stats', path: 'stats',
            tool: 'mgr_team_gui' },
        { id: 'scores', title: 'Scorecard', kind: 'stats', path: 'scores',
            tool: 'mgr_team_gui' },
        {
            id: 'team', title: 'Team', kind: 'table', path: 'team',
            tool: 'mgr_team_gui', empty: 'No team',
            // The five numbers flight risk is DERIVED from are each a column,
            // because a risk score with nothing behind it is a number nobody
            // can act on - and the whole point is that a manager can move
            // every one of them.
            columns: [col('name', 'Name'), col('role', 'Role'),
                col('level', 'Level'), col('availability', 'Availability'),
                col('capacity', 'Capacity', 'number'),
                col('morale', 'Morale', 'number'),
                col('engagement', 'Engagement', 'number'),
                col('growth', 'Growth', 'number'),
                col('burnout', 'Burnout', 'number'),
                col('risk', 'Flight risk', 'badge'), col('cadence', '1:1'),
                col('overdue', 'Overdue', 'number'),
                col('feedback', 'Evidence', 'number'),
                col('skills', 'Skills'), col('status', 'Status', 'badge'),
                col('why', 'Why')],
        },
        {
            id: 'oneonones', title: 'One-to-ones', kind: 'table',
            path: 'oneonones', tool: 'mgr_team_gui', empty: 'None scheduled',
            columns: [col('name', 'With'), col('cadence', 'Cadence'),
                col('last', 'Last held (sprint)', 'number'),
                col('overdue', 'Sprints overdue', 'number'),
                col('held', 'Held', 'number'),
                col('actions', 'With an action', 'number'),
                col('status', 'Status', 'badge')],
        },
        {
            id: 'performance', title: 'Performance', kind: 'table',
            path: 'performance', tool: 'mgr_team_gui', empty: 'Nothing rated',
            columns: [col('name', 'Name'), col('level', 'Level'),
                col('box', 'Impact / trajectory'),
                col('evidence', 'Specific feedback', 'number'),
                col('defensible', 'Defensible', 'badge'),
                col('at_level', 'Sprints at level', 'number'),
                col('promo', 'Promotion'), col('status', 'Status', 'badge')],
        },
        {
            id: 'board', title: 'Board', kind: 'table', path: 'board',
            tool: 'mgr_delivery_gui', empty: 'Empty backlog',
            // `cost` is the points the work will ACTUALLY take given who it is
            // assigned to. It is the column that makes a skills read a
            // planning input rather than a nicety.
            columns: [col('id', 'Id'), col('title', 'Title'),
                col('points', 'Points', 'number'), col('skill', 'Skill'),
                col('value', 'Value', 'badge'), col('status', 'State', 'badge'),
                col('assignee', 'Assignee'), col('cost', 'Real cost', 'number'),
                col('carried', 'Carried', 'number'), col('kr', 'Key result'),
                col('sprint', 'Sprint', 'number')],
        },
        {
            id: 'sprints', title: 'Sprints', kind: 'table', path: 'sprints',
            tool: 'mgr_delivery_gui', empty: 'No sprints',
            columns: [col('n', 'Sprint', 'number'), col('status', 'Status', 'badge'),
                col('capacity', 'Capacity', 'number'),
                col('committed', 'Committed', 'number'),
                col('delivered', 'Delivered', 'number'),
                col('carried', 'Carried', 'number'), col('say_do', 'Say / do'),
                col('items', 'Items', 'number')],
        },
        {
            id: 'okrs', title: 'OKRs', kind: 'table', path: 'okrs',
            tool: 'mgr_delivery_gui', empty: 'No objectives',
            columns: [col('objective', 'Objective'), col('id', 'KR'),
                col('kr', 'Key result'), col('owner', 'Owner'),
                col('baseline', 'Baseline', 'number'),
                col('current', 'Now', 'number'), col('target', 'Target', 'number'),
                col('progress', 'Progress'),
                col('confidence', 'Confidence', 'number'),
                col('checkins', 'Check-ins', 'number'),
                col('status', 'Status', 'badge')],
        },
        {
            id: 'raci', title: 'RACI', kind: 'table', path: 'raci',
            tool: 'mgr_delivery_gui', empty: 'No RACI recorded',
            columns: [col('item', 'Item'), col('responsible', 'Responsible', 'list'),
                col('accountable', 'Accountable', 'list'),
                col('consulted', 'Consulted', 'list'),
                col('informed', 'Informed', 'list'),
                col('status', 'Valid', 'badge')],
        },
        {
            id: 'raid', title: 'RAID register', kind: 'table', path: 'raid',
            tool: 'mgr_risk_gui', empty: 'The register is empty - which is not '
                                      + 'the same as having no risks',
            columns: [col('id', 'Id'), col('kind', 'Kind', 'badge'),
                col('title', 'Title'), col('owner', 'Owner'),
                col('impact', 'Impact', 'badge'),
                col('likelihood', 'Likelihood', 'badge'), col('due', 'Due'),
                col('mitigation', 'Mitigation'),
                col('age', 'Sprints open', 'number'),
                col('controlled', 'Controlled', 'badge'),
                col('status', 'Status', 'badge')],
        },
        {
            id: 'incidents', title: 'Incidents', kind: 'table',
            path: 'incidents', tool: 'mgr_risk_gui', empty: 'No incidents',
            columns: [col('id', 'Id'), col('severity', 'Severity', 'badge'),
                col('title', 'Title'), col('sprint', 'Sprint', 'number'),
                col('mtta', 'MTTA min', 'number'), col('mttr', 'MTTR min', 'number'),
                col('postmortem', 'Postmortem', 'badge'),
                col('actions', 'Actions', 'number'),
                col('blameless', 'Blameless', 'badge'),
                col('source', 'Came from'), col('status', 'Status', 'badge')],
        },
        {
            id: 'decisions', title: 'Decision log', kind: 'table',
            path: 'decisions', tool: 'mgr_risk_gui',
            empty: 'No decisions recorded - every one you make is being '
                 + 'forgotten',
            columns: [col('id', 'Id'), col('sprint', 'Sprint', 'number'),
                col('title', 'Decision'), col('options', 'Alternatives', 'number'),
                col('chosen', 'Chosen'), col('because', 'Why'),
                col('door', 'Door', 'badge'), col('consulted', 'Consulted', 'list'),
                col('status', 'Status', 'badge')],
        },
        {
            id: 'stakeholders', title: 'Stakeholders', kind: 'table',
            path: 'stakeholders', tool: 'mgr_org_gui', empty: 'No stakeholders',
            columns: [col('name', 'Name'), col('role', 'Role'),
                col('power', 'Power', 'badge'), col('interest', 'Interest', 'badge'),
                col('sentiment', 'Confidence', 'number'),
                col('since', 'Sprints since contact', 'number'),
                col('briefs', 'Briefings', 'number'), col('wants', 'What they want'),
                col('status', 'Status', 'badge')],
        },
        {
            id: 'hiring', title: 'Hiring funnel', kind: 'table', path: 'hiring',
            tool: 'mgr_org_gui', empty: 'No open requisitions',
            columns: [col('id', 'Id'), col('name', 'Candidate'),
                col('role', 'Role'), col('stage', 'Stage', 'badge'),
                col('in_stage', 'Sprints in stage', 'number'),
                col('cards', 'Scorecards', 'number'),
                col('evidenced', 'With evidence', 'number'),
                col('signals', 'Signals'), col('status', 'Status', 'badge')],
        },
        {
            id: 'cost', title: 'Budget', kind: 'table', path: 'cost',
            tool: 'mgr_org_gui', empty: 'No cost lines',
            columns: [col('line', 'Line'), col('monthly', 'Per month', 'number'),
                col('share', 'Share'), col('tagged', 'Tagged', 'badge'),
                col('note', 'Note')],
        },
        {
            id: 'signals', title: 'What the simulation noticed', kind: 'log',
            path: 'signals', tool: 'mgr_org_gui',
            empty: 'Nothing yet - signals appear when you `mgr advance`',
        },
        {
            id: 'retros', title: 'Retros', kind: 'table', path: 'retros',
            tool: 'mgr_org_gui', empty: 'No retros',
            columns: [col('sprint', 'Sprint', 'number'), col('keep', 'Keep'),
                col('change', 'Change'), col('action', 'Action')],
        },
    ],
};
