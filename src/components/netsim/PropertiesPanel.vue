<template>
  <aside class="ns-props">
    ...
    <div v-if="!device && !link" class="ns-props-empty">
      ...
      <p>Click a device to configure it, or a cable to inspect the link.</p>
      <div class="ns-quick-stats">
        <div><strong>{{ store.stats.devices }}</strong><span>devices</span></div>
        <div><strong>{{ store.stats.links }}</strong><span>links</span></div>
        <div><strong>{{ store.stats.vlans }}</strong><span>VLANs</span></div>
        <div :class="{ bad: store.errorCount > 0 }"><strong>{{ store.errorCount }}</strong><span>errors</span></div>
      </div>
    </div>
    ...
    <div v-else-if="link" class="ns-props-inner">
      <header class="ns-props-head">
        ...
        <button class="ns-icon-btn" title="Close" @click="store.selectLink(null)"><DeviceIcon name="close" :size="15" /></button>
      </header>

      <div class="ns-props-body">
        <div class="ns-field-grid">
          <label>Status<span class="ns-status-pill" :class="link.severed ? 'down' : link.status">{{ link.severed ? 'cut' : link.status }}</span></label>
          <label>Latency
          ...
          <label>Label
            <input type="text" v-model="link.label" placeholder="e.g. 802.1Q trunk" @change="store.markDirty()" />
          </label>
          <label>Cable type
        ...
        <div class="ns-btn-row">
          ...
          <button class="ns-btn danger" @click="store.removeLink(link.id)">
            <DeviceIcon name="trash" :size="14" /> Delete
          </button>
        </div>
      </div>
    </div>
    ...
    <div v-else-if="device" class="ns-props-inner">
      <header class="ns-props-head">
        ...
        <div>
          ...
          <p>{{ type?.name }} · Layer {{ type?.layer }} · {{ type?.role }}</p>
        </div>
        <button class="ns-icon-btn" title="Close" @click="store.select(null)"><DeviceIcon name="close" :size="15" /></button>
      </header>
      ...
      <div class="ns-props-body">
        ...
        <section v-if="tab === 'overview'">
          <div class="ns-btn-row tight">
            ...
            <button class="ns-btn ghost" @click="emit('open-terminal', device.id)">
              <DeviceIcon name="terminal" :size="14" /> Terminal
            </button>
            <button class="ns-btn ghost" @click="store.duplicateDevice(device.id)">
              <DeviceIcon name="copy" :size="14" /> Duplicate
            </button>
          ...
          <div class="ns-btn-row tight">
            <button class="ns-btn ghost sm" @click="doPing">Ping…</button>
            <button v-if="canDhcp" class="ns-btn ghost sm" @click="store.runDhcp(device.id)">Request DHCP</button>
            <button v-if="isHost" class="ns-btn ghost sm" @click="doHttp">Browse…</button>
            <button v-if="isHost" class="ns-btn ghost sm" @click="doDns">nslookup…</button>
          </div>

          <div class="ns-info-rows">
            <div><span>Reachable addresses</span><strong>{{ addressSummary || '—' }}</strong></div>
            <div><span>Cabled ports</span><strong>{{ cabledCount }} / {{ device.interfaces.filter(i => i.medium !== 'console').length }}</strong></div>
            <div v-if="isHost"><span>Default gateway</span><strong :class="{ bad: gatewayBad }">{{ device.host.defaultGateway || 'not set' }}</strong></div>
            <div v-if="isHost"><span>DNS server</span><strong>{{ device.host.dnsServer || 'not set' }}</strong></div>
            <div v-if="device.stp?.enabled"><span>Spanning tree</span><strong>{{ device.stp.isRoot ? 'root bridge' : device.stp.mode.toUpperCase() }} · priority {{ device.stp.priority }}</strong></div>
          </div>

          <label class="ns-field block">Notes
          ...
          <div v-if="type?.learn?.length" class="ns-learn-block">
            <h5><DeviceIcon name="book" :size="14" /> About this device</h5>
            ...
            >
              ...
              <div v-show="openIface === st.iface.id" class="ns-iface-body">
                <p class="ns-iface-meta">
                  {{ st.iface.name }} · {{ st.iface.medium }} · {{ formatSpeed(st.iface.speedMbps) }} · MAC {{ st.iface.mac }}<br />
                  <span :class="{ bad: !st.status.startsWith('up/up') }">{{ st.status }}</span>
                  <template v-if="st.linkTo"> → {{ st.linkTo }}</template>
                  <template v-if="st.stpRole"> · STP {{ st.stpRole }}</template>
                </p>
                ...
                <div class="ns-field-grid">
                  <label class="ns-check">
                    ...
                    <span>Interface enabled (no shutdown)</span>
                  </label>
                  <label class="ns-check">
                    <input type="checkbox" v-model="st.iface.dhcp" @change="store.applyConfigChange()" />
                    <span>Obtain address by DHCP</span>
                  </label>
                  <label :class="{ disabled: st.iface.dhcp }">IPv4 address
                  ...
                  <label :class="{ disabled: st.iface.dhcp }">Subnet mask
                  ...
                  <label>IPv6 address
                    <input type="text" v-model="st.iface.ipv6" placeholder="2001:db8:10::1" spellcheck="false" @change="store.applyConfigChange()" />
                  </label>
                  <label>IPv6 prefix
                    <input type="number" min="1" max="128" v-model.number="st.iface.prefix6" @change="store.applyConfigChange()" />
                  </label>
                  <label>Description
                    <input type="text" v-model="st.iface.description" placeholder="to SW1 Gi1/0/1" @change="store.markDirty()" />
                  </label>
                  ...
                  <template v-if="st.iface.medium === 'wireless' || st.iface.medium === 'cellular'">
                    <label class="span2">Join SSID
                      <input type="text" v-model="st.iface.ssid" placeholder="SelfStudy-WiFi" spellcheck="false" @change="store.applyConfigChange()" />
                    </label>
                    <label class="span2">Passphrase
                      <input type="text" v-model="st.iface.passphrase" placeholder="matches the AP exactly" spellcheck="false" @change="store.applyConfigChange()" />
                    </label>
                    <label>Band
                      <select v-model="st.iface.band" @change="store.applyConfigChange()">
                        <option value="2.4GHz">2.4 GHz</option><option value="5GHz">5 GHz</option><option value="6GHz">6 GHz</option>
                      </select>
                    </label>
                  </template>
                </div>
                ...
                <div v-if="st.iface.ipv4 && subnetOf(st.iface)" class="ns-subnet-facts">
                  <span>net <strong>{{ subnetOf(st.iface)!.network }}/{{ subnetOf(st.iface)!.prefix }}</strong></span>
                  <span>hosts <strong>{{ subnetOf(st.iface)!.firstHost }}–{{ subnetOf(st.iface)!.lastHost }}</strong></span>
                  <span>bcast <strong>{{ subnetOf(st.iface)!.broadcast }}</strong></span>
                  <span>{{ subnetOf(st.iface)!.hosts }} usable · {{ subnetOf(st.iface)!.scope }}</span>
                </div>

                <div v-if="st.iface.counters" class="ns-counters">
                  rx {{ st.iface.counters.rxFrames }} frames / {{ st.iface.counters.rxBytes }} B ·
                  tx {{ st.iface.counters.txFrames }} frames / {{ st.iface.counters.txBytes }} B ·
                  drops {{ st.iface.counters.drops }}
                </div>
              </div>
            </div>
          </div>
        ...
        <section v-else-if="tab === 'host'">
          ...
          <div v-if="gatewayBad" class="ns-note-block bad">
            <strong>Problem:</strong> the gateway {{ device.host.defaultGateway }} is not inside any of this host's own subnets. A host cannot even ARP for an address outside its subnet — this is nearly always a wrong subnet mask.
          </div>
          <div class="ns-note-block">
            <strong>Remember:</strong> a host compares the destination against its own address using the mask. Same network → ARP directly. Different network → send the frame to the gateway's MAC while the IP header still targets the final destination.
          </div>
        </section>
        ...
        <section v-else-if="tab === 'vlans'">
          <div class="ns-row-head">
            <h5>VLAN database</h5>
            <button class="ns-btn ghost sm" @click="addVlanRow"><DeviceIcon name="plus" :size="13" /> Add VLAN</button>
          </div>
          <table class="ns-table">
            <thead><tr><th>ID</th><th>Name</th><th>Access ports</th><th></th></tr></thead>
            <tbody>
              <tr v-for="v in device.vlans" :key="v.id">
                ...
                <td>
                  <button v-if="v.id !== 1" class="ns-icon-btn danger" title="Delete VLAN" @click="removeVlan(v.id)">
          ...
          <template v-if="isMultilayer">
            <div class="ns-row-head">
              <h5>SVIs (inter-VLAN routing)</h5>
              <button class="ns-btn ghost sm" @click="addSviRow"><DeviceIcon name="plus" :size="13" /> Add SVI</button>
            </div>
            <table class="ns-table">
              <thead><tr><th>Interface</th><th>Address</th><th>Mask</th><th></th></tr></thead>
              <tbody>
                ...
                <tr v-if="!svis.length"><td colspan="4" class="ns-muted">No SVIs yet. Each VLAN needs one to be routable.</td></tr>
              </tbody>
            </table>
          </template>
          ...
          <div v-if="device.stp" class="ns-sub-block">
            ...
            <div class="ns-field-grid">
              <label class="ns-check span2">
                ...
                <span>Spanning tree enabled</span>
              </label>
              <label>Mode
                <select v-model="device.stp.mode" @change="store.applyConfigChange()">
                  <option value="stp">stp</option><option value="rstp">rstp</option>
                  <option value="pvst">pvst</option><option value="rapid-pvst">rapid-pvst</option><option value="mstp">mstp</option>
                </select>
              </label>
            ...
            <p class="ns-muted sm">
              Lowest priority wins the root election; the MAC address breaks ties. Set this deliberately on the switch you
              want as root — leaving every switch at 32768 means the oldest box wins by accident.
            </p>
          </div>
        </section>
        ...
        <section v-else-if="tab === 'routing'">
          <div class="ns-row-head">
            <h5>Static routes</h5>
            <button class="ns-btn ghost sm" @click="addRoute"><DeviceIcon name="plus" :size="13" /> Add route</button>
          </div>
          <table class="ns-table">
            <thead><tr><th>Network</th><th>Mask</th><th>Next hop</th><th></th></tr></thead>
            <tbody>
              ...
              <tr v-if="!device.routing.staticRoutes.length"><td colspan="4" class="ns-muted">None. A router only knows its connected networks until you add routes.</td></tr>
            </tbody>
          </table>

          <div class="ns-field-grid">
            <label class="span2">Default route / gateway of last resort
          ...
          <div class="ns-sub-block">
            ...
            <div class="ns-field-grid">
              <label class="ns-check span2">
                ...
                <span>OSPF (link-state, AD 110, bandwidth-based cost)</span>
              </label>
              <label v-if="device.routing.ospf.enabled">Process ID
                <input type="number" min="1" v-model.number="device.routing.ospf.processId" @change="store.applyConfigChange()" />
              </label>
              <label v-if="device.routing.ospf.enabled">Router ID
              ...
              <label class="ns-check span2">
                ...
                <span>RIP (distance-vector, AD 120, max 15 hops)</span>
              </label>
              <label v-if="device.routing.rip.enabled">RIP version
                <select v-model.number="device.routing.rip.version" @change="store.applyConfigChange()">
                  <option :value="1">1 (classful)</option><option :value="2">2 (classless, VLSM)</option>
                </select>
              </label>
            </div>
          </div>
          ...
          <div class="ns-sub-block">
            <h5>Routing table (live)</h5>
            <table class="ns-table mono">
              <thead><tr><th>Code</th><th>Destination</th><th>Via</th><th>AD/Metric</th></tr></thead>
              <tbody>
                ...
                <tr v-if="!routingTable.length"><td colspan="4" class="ns-muted">Empty — address and enable the interfaces first.</td></tr>
              </tbody>
            </table>
          </div>
        </section>
        ...
        <section v-else-if="tab === 'services'">
          <div class="ns-service-toggle">
            <label class="ns-check">
              ...
              <span>HTTP server (port {{ device.services.http.port }})</span>
            </label>
            <template v-if="device.services.http.enabled">
              <div class="ns-field-grid indent">
                <label>Port<input type="number" v-model.number="device.services.http.port" @change="store.applyConfigChange()" /></label>
                <label class="ns-check"><input type="checkbox" v-model="device.services.http.tls" @change="store.applyConfigChange()" /><span>Also serve HTTPS (443)</span></label>
                <label class="span2">Page title<input type="text" v-model="device.services.http.title" @change="store.markDirty()" /></label>
                <label class="span2">Page body (HTML)<textarea rows="3" v-model="device.services.http.body" @change="store.markDirty()"></textarea></label>
              </div>
            </template>
          </div>
          ...
          <div class="ns-service-toggle">
            <label class="ns-check">
              ...
              <span>DNS server (UDP 53)</span>
            </label>
            <template v-if="device.services.dns.enabled">
              <div class="ns-row-head indent">
                <span class="ns-muted sm">Authoritative records</span>
                <button class="ns-btn ghost sm" @click="addDnsRow"><DeviceIcon name="plus" :size="13" /> Add record</button>
              </div>
              <table class="ns-table indent">
                ...
                <tbody>
                  <tr v-for="r in device.services.dns.records" :key="r.id">
                    <td><input class="ns-inline-input" v-model="r.name" placeholder="www.lab.local" @change="store.applyConfigChange()" /></td>
                    <td>
                  ...
                  <tr v-if="!device.services.dns.records.length"><td colspan="4" class="ns-muted">No records. Without one, clients get NXDOMAIN.</td></tr>
                </tbody>
              </table>
            </template>
          </div>
          ...
          <div class="ns-service-toggle">
            <label class="ns-check">
              ...
              <span>DHCP server (UDP 67)</span>
            </label>
            <template v-if="device.services.dhcp.enabled">
              <div class="ns-row-head indent">
                <span class="ns-muted sm">Address pools</span>
                <button class="ns-btn ghost sm" @click="addPool"><DeviceIcon name="plus" :size="13" /> Add pool</button>
              </div>
              <div v-for="p in device.services.dhcp.pools" :key="p.id" class="ns-pool indent">
                ...
                <div class="ns-field-grid">
                  <label>Network<input v-model="p.network" placeholder="10.1.10.0" @change="store.applyConfigChange()" /></label>
                  <label>Mask<input v-model="p.mask" placeholder="255.255.255.0" @change="store.applyConfigChange()" /></label>
                  <label>Range start<input v-model="p.rangeStart" placeholder="10.1.10.100" @change="store.applyConfigChange()" /></label>
                  <label>Range end<input v-model="p.rangeEnd" placeholder="10.1.10.200" @change="store.applyConfigChange()" /></label>
                  <label>Gateway (opt 3)<input v-model="p.gateway" placeholder="10.1.10.1" @change="store.applyConfigChange()" /></label>
                  <label>DNS (opt 6)<input v-model="p.dnsServer" placeholder="10.1.30.10" @change="store.applyConfigChange()" /></label>
                  <label>Lease (hours)<input type="number" min="1" v-model.number="p.leaseHours" @change="store.markDirty()" /></label>
                </div>
              </div>
              <div v-if="leases.length" class="ns-sub-block indent">
                <h5>Active leases</h5>
                <table class="ns-table mono">
                  <thead><tr><th>IP</th><th>MAC</th><th>Host</th></tr></thead>
          ...
          <div class="ns-service-toggle compact">
            <label class="ns-check"><input type="checkbox" v-model="device.services.ftp.enabled" @change="store.applyConfigChange()" /><span>FTP (21)</span></label>
            <label class="ns-check"><input type="checkbox" v-model="device.services.smtp.enabled" @change="store.applyConfigChange()" /><span>SMTP (25)</span></label>
            <label class="ns-check"><input type="checkbox" v-model="device.services.ntp.enabled" @change="store.applyConfigChange()" /><span>NTP (123)</span></label>
            <label class="ns-check"><input type="checkbox" v-model="device.services.syslog.enabled" @change="store.applyConfigChange()" /><span>Syslog (514)</span></label>
            <label class="ns-check"><input type="checkbox" v-model="device.services.mqtt.enabled" @change="store.applyConfigChange()" /><span>MQTT (1883)</span></label>
            <label class="ns-check"><input type="checkbox" v-model="device.services.radius.enabled" @change="store.applyConfigChange()" /><span>RADIUS (1812)</span></label>
          </div>
        </section>
        ...
        <section v-else-if="tab === 'security'">
          <div class="ns-row-head">
            ...
            <button class="ns-btn ghost sm" @click="addAclRow"><DeviceIcon name="plus" :size="13" /> New ACL</button>
          </div>
          ...
          <div v-for="acl in device.acls" :key="acl.id" class="ns-acl">
            <div class="ns-acl-head">
              ...
              <select class="ns-inline-input" v-model="acl.type" @change="store.applyConfigChange()">
                <option value="standard">standard</option><option value="extended">extended</option>
              </select>
            ...
            <table class="ns-table">
              <thead><tr><th>#</th><th>Action</th><th>Proto</th><th>Source</th><th v-if="acl.type === 'extended'">Destination</th><th v-if="acl.type === 'extended'">Port</th><th>Hits</th><th></th></tr></thead>
              <tbody>
                <tr v-for="r in [...acl.rules].sort((a, b) => a.seq - b.seq)" :key="r.id">
                  ...
                  <td class="ns-acl-addr">
                    <label class="ns-check tiny"><input type="checkbox" v-model="r.srcAny" @change="store.applyConfigChange()" /><span>any</span></label>
                  ...
                  <td v-if="acl.type === 'extended'" class="ns-acl-addr">
                    <label class="ns-check tiny"><input type="checkbox" v-model="r.dstAny" @change="store.applyConfigChange()" /><span>any</span></label>
                ...
                <tr class="ns-implicit"><td colspan="8">implicit deny any — every ACL ends with this</td></tr>
              </tbody>
            </table>
            <button class="ns-btn ghost sm" @click="addAclRule(acl.id)"><DeviceIcon name="plus" :size="12" /> Add rule</button>
          </div>
          <p v-if="!device.acls.length" class="ns-muted">No ACLs. Create one, add rules, then apply it to an interface on the Interfaces tab.</p>
          ...
          <div v-if="supportsNat" class="ns-sub-block">
            ...
            <div class="ns-field-grid">
              ...
              <label>Mode
                <select v-model="device.nat.mode" @change="store.applyConfigChange()">
                  <option value="pat">PAT (overload) — many to one</option>
                  <option value="dynamic">dynamic pool</option>
                  <option value="static">static one-to-one</option>
                </select>
              </label>
            ...
            <p class="ns-muted sm">Mark interfaces as NAT inside / outside on the Interfaces tab, or nothing is translated.</p>
            <div v-if="device.nat.translations?.length" class="ns-sub-block">
              <table class="ns-table mono">
                <thead><tr><th>Pro</th><th>Inside local</th><th>Inside global</th><th>Outside</th></tr></thead>
                <tbody>
        ...
        <section v-else-if="tab === 'wireless' && device.wireless">
          <div class="ns-field-grid">
            ...
            <label class="span2">Guest SSID (optional)
            ...
            <label>Security
              <select v-model="device.wireless.security" @change="store.applyConfigChange()">
                ...
                <option value="wep">WEP (broken — never use)</option>
            ...
            <label>Band
              <select v-model="device.wireless.band" @change="store.applyConfigChange()">
                <option value="2.4GHz">2.4 GHz (range)</option>
                <option value="5GHz">5 GHz (capacity)</option>
                <option value="6GHz">6 GHz (Wi-Fi 6E/7, WPA3 only)</option>
              </select>
            </label>
            <label>Channel
            ...
            <label>Channel width (MHz)
              <select v-model.number="device.wireless.channelWidthMHz" @change="store.applyConfigChange()">
                ...
                <option :value="160">160</option><option :value="320">320 (Wi-Fi 7, 6 GHz)</option>
              </select>
            </label>
            <label>Standard
            ...
            <label>Mapped VLAN
              <input type="number" min="1" max="4094" v-model.number="device.wireless.vlanId" placeholder="untagged" @change="store.applyConfigChange()" />
            </label>
            <label>TX power (dBm)
            ...
            <label class="ns-check span2">
              ...
              <span>Hide the SSID (security theatre — it does not stop anyone)</span>
            </label>
          </div>
          ...
          <div class="ns-sub-block">
            <h5>Associated clients ({{ associatedClients.length }})</h5>
        ...
        <section v-else-if="tab === 'tables'">
          <div v-if="supportsVlans" class="ns-sub-block">
            <div class="ns-row-head">
              <h5>MAC address table</h5>
              <button class="ns-btn ghost sm" @click="clearMac">Clear</button>
            </div>
            <table class="ns-table mono">
              <thead><tr><th>VLAN</th><th>MAC</th><th>Type</th><th>Port</th></tr></thead>
              <tbody>
                <tr v-for="(e, i) in device.macTable || []" :key="i">
                  <td>{{ e.vlan }}</td><td>{{ e.mac }}</td><td>{{ e.type }}</td><td>{{ portName(e.interfaceId) }}</td>
                </tr>
                <tr v-if="!(device.macTable || []).length"><td colspan="4" class="ns-muted">Empty — a switch only learns from frames it receives.</td></tr>
              </tbody>
            </table>
          </div>
          ...
          <div class="ns-sub-block">
            <div class="ns-row-head">
              <h5>ARP cache</h5>
              <button class="ns-btn ghost sm" @click="clearArp">Clear</button>
            </div>
            <table class="ns-table mono">
              ...
              <tbody>
                ...
                <tr v-if="!(device.arpTable || []).length"><td colspan="4" class="ns-muted">Empty — ARP is populated on demand, when traffic needs it.</td></tr>
              </tbody>
            </table>
          </div>

          <div class="ns-sub-block">
            <div class="ns-row-head"><h5>Running configuration</h5>
              <button class="ns-btn ghost sm" @click="copyConfig"><DeviceIcon name="copy" :size="12" /> Copy</button>
            </div>
