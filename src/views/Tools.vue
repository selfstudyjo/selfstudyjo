<template>
  <div class="labs-container">
    ...
    <div v-if="loading" class="loading-state">
      ...
      <p>{{ $t('Starting your workspace...') }}</p>
    </div>

    <!-- Error State -->
    <div v-else-if="error" class="error-state">
      <AlertTriangle class="lab-i lab-i--lg" />
      <h3>{{ $t('These tools are not reachable right now') }}</h3>
      <p>{{ error }}</p>
...
/*
  Which sandbox is open. Mirrored into the URL as `/tools/<tab>` so the sidebar
  can link straight to one, a reload comes back to the same place, and a
  student can send a classmate the tab they are stuck on. `/tools` with no
  segment is SQL.
*/
...
function selectTab(id: TabId) {
  ...
  router.push(id === 'sql' ? '/tools' : `/tools/${id}`);
}
