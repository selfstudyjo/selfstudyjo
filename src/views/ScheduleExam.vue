  ...
  <div class="schedule-exam-page">
    <div class="page-header">
      <div class="header-content">
        ...
        <button @click="$router.back()" class="back-btn">
          ...
          {{ $t('Back to {v0}', { v0: isReschedule ? 'Appointment' : 'Exams' }) }}
        </button>
      </div>
    </div>
    ...
    <div class="progress-steps" v-if="!isReschedule">
      <div class="steps-container">
        <div class="step" :class="{ active: currentStep === 1, completed: currentStep > 1 }">
          ...
          <div class="step-label">{{ $t('Select Exam') }}</div>
        </div>
        <div class="step-line"></div>
        <div class="step" :class="{ active: currentStep === 2, completed: currentStep > 2 }">
          <div class="step-number">2</div>
          <div class="step-label">{{ $t('Select Proctor') }}</div>
        </div>
        <div class="step-line"></div>
        <div class="step" :class="{ active: currentStep === 3, completed: currentStep > 3 }">
          <div class="step-number">3</div>
          <div class="step-label">{{ $t('Select Date & Time') }}</div>
        </div>
        <div class="step-line"></div>
        <div class="step" :class="{ active: currentStep === 4, completed: currentStep > 4 }">
          <div class="step-number">4</div>
          <div class="step-label">{{ $t('Confirm Booking') }}</div>
        </div>
      </div>
    </div>
    ...
    <div class="progress-steps" v-else>
      <div class="steps-container">
        <div class="step" :class="{ active: currentStep === 1, completed: currentStep > 1 }">
          <div class="step-number">1</div>
          <div class="step-label">{{ $t('Current Appointment') }}</div>
        </div>
        <div class="step-line"></div>
        <div class="step" :class="{ active: currentStep === 2, completed: currentStep > 2 }">
          <div class="step-number">2</div>
          <div class="step-label">{{ $t('Select New Date & Time') }}</div>
        </div>
        <div class="step-line"></div>
        <div class="step" :class="{ active: currentStep === 3, completed: currentStep > 3 }">
          <div class="step-number">3</div>
          <div class="step-label">{{ $t('Confirm Changes') }}</div>
        </div>
      </div>
    </div>
    ...
    <div v-else-if="error" class="error-container">
      ...
      <h3>{{ $t('Error') }}</h3>
      <p>{{ error }}</p>
      <button @click="retryCurrentStep" class="retry-btn">{{ $t('Retry') }}</button>
    </div>

    <!-- Step 1: Select Exam (only for new scheduling) -->
    <div v-else-if="!isReschedule && currentStep === 1" class="step-content">
      <div class="step-header">
        <h2>{{ $t('Select an Exam') }}</h2>
        <p>{{ $t('Choose the exam you want to schedule') }}</p>
      </div>
        ...
        <div v-for="exam in availableExams" :key="exam.external_id"
          ...
          <div class="exam-option-details">
            ...
            <div v-else-if="hasActiveAppointment(exam)" class="exam-status scheduled">
              ...
              {{ $t('Already scheduled for {v0}', { v0: getScheduledDate(exam) }) }}
            </div>
            <div v-else-if="hasFailedAppointment(exam)" class="exam-status failed">
              ...
              {{ $t('Previous attempt failed - You can reschedule') }}
            </div>
          </div>
          <div class="exam-option-check">
            <div class="check-circle" v-if="selectedExam?.external_id === exam.external_id">✓</div>
            <div v-else-if="hasExamPassed(exam)" class="status-tag passed">{{ $t('PASSED') }}</div>
            <div v-else-if="hasActiveAppointment(exam)" class="status-tag scheduled">{{ $t('BOOKED') }}</div>
            <div v-else-if="hasFailedAppointment(exam)" class="status-tag failed">{{ $t('RETRY') }}</div>
          </div>
        </div>
      </div>

      <div class="step-actions">
        <button @click="cancelSchedule" class="btn-cancel">{{ $t('Cancel') }}</button>
        <button @click="nextStep" :disabled="!selectedExam || !canScheduleExam(selectedExam)" class="btn-next">
          {{ $t('Next: Select Proctor') }}
          <svg width="16" height="16" viewBox="0 0 24 24" fill="none">
    ...
    <div v-else-if="!isReschedule && currentStep === 2" class="step-content">
      <div class="step-header">
        <h2>{{ $t('Select a Proctor') }}</h2>
        <p>{{ $t('Choose an available proctor for your exam') }}</p>
      </div>
    ...
    <div v-else-if="isReschedule && currentStep === 1" class="step-content">
      <div class="step-header">
        <h2>{{ $t('Current Appointment Details') }}</h2>
        <p>{{ $t('Review your current exam appointment') }}</p>
      </div>
        ...
        <div class="appointment-card">
            ...
            <div class="detail-section">
              ...
              <div class="detail-grid">
                ...
                <div class="detail-item">
                  ...
                  <span class="value">{{ $t('{v0} minutes', { v0: selectedExam?.exam_duration }) }}</span>
                </div>
              </div>
            </div>

            <div class="detail-section">
              <h4>{{ $t('Proctor Information') }}</h4>
              <div class="detail-grid">
                <div class="detail-item">
                  <span class="label">{{ $t('Proctor:') }}</span>
                ...
                <div class="detail-item">
                  ...
                  <span class="value status-active">{{ $t('Active') }}</span>
                </div>
              </div>
            </div>

            <div class="detail-section">
              <h4>{{ $t('Current Schedule') }}</h4>
              <div class="detail-grid">
                <div class="detail-item">
          ...
          <div class="reschedule-notice">
            ...
            <div class="notice-content">
              ...
              <ul>
                <li>{{ $t('You can reschedule up to 2 days before your current appointment') }}</li>
                <li>{{ $t('The old time slot will become available for other students') }}</li>
                <li>{{ $t('You will create a new appointment with the selected date/time') }}</li>
                <li>{{ $t('The old appointment will be cancelled automatically') }}</li>
              </ul>
            </div>
          </div>
        </div>
    ...
    <div v-else-if="isDateStep" class="step-content">
      <div class="step-header">
        <h2>{{ $t('Select Date & Time') }}</h2>
        <p>{{ $t('Choose an available time slot for your exam') }}</p>
        <div v-if="hasActiveAppointmentOnSelectedDate && !isReschedule" class="date-warning">
          ...
          <span>{{ $t('You already have an appointment on this date. Please select another date.') }}</span>
        </div>
      </div>
      ...
      <div v-if="!selectedProctor" class="proctor-reselect">
        ...
        <div class="proctors-grid">
          ...
          <div v-if="proctors.length === 0" class="no-slots">
            <p>{{ $t('No proctors are available right now.') }}</p>
            <p>{{ $t('Please try again later or contact an administrator.') }}</p>
          </div>
        </div>
      </div>
      ...
      <div v-else class="date-time-selector">
        ...
        <div class="date-selector">
          ...
          <div class="availability-note">
            <span v-if="availabilityLoading">{{ $t('Checking {v0}\'s availability…', { v0: selectedProctor?.username }) }}</span>
            <span v-else-if="availabilityError" class="availability-note--error">
              {{ availabilityError }}
              <button type="button" class="link-btn" @click="retryAvailability">{{ $t('Retry') }}</button>
            </span>
            <span v-else-if="bookableDatesInMonth > 0">
              {{ $t('{v0} has {v1} bookable {v2} in {v3}.', { v0: selectedProctor?.username, v1: bookableDatesInMonth, v2: bookableDatesInMonth === 1 ? 'date' : 'dates', v3: currentMonth }) }}
            </span>
        ...
        <div class="time-selector" v-if="selectedDate && (!hasActiveAppointmentOnSelectedDate || isReschedule)">
          <h3>
            ...
            <span v-if="slotsLoading" class="slots-refreshing">{{ $t('refreshing…') }}</span>
          </h3>
          <div class="time-slots">
            ...
            <div v-if="availableTimeSlots.length === 0 && !slotsLoading" class="no-slots">
              <p>{{ $t('Every slot on this date has just been taken') }}</p>
              <p>{{ $t('Pick another date marked ✓ on the calendar') }}</p>
            </div>
          </div>
        </div>

        <div v-else-if="selectedDate && hasActiveAppointmentOnSelectedDate && !isReschedule" class="time-selector placeholder error">
          <h3>{{ $t('Date Not Available') }}</h3>
          <div class="placeholder-message">
            <div class="error-icon">⚠️</div>
            <p>{{ $t('You already have an appointment on this date.') }}</p>
            <p>{{ $t('Please select another date to schedule your exam.') }}</p>
          </div>
        </div>
        ...
        <div v-else class="time-selector placeholder">
          ...
          <div class="placeholder-message">
            <p>{{ $t('Please select a date to see available time slots') }}</p>
          </div>
        </div>
      </div>
    ...
    <div v-else-if="(!isReschedule && currentStep === 4) || (isReschedule && currentStep === 3)" class="step-content">
      ...
      <div class="booking-summary">
        ...
        <div v-if="isReschedule && existingAppointment" class="current-appointment-section">
          <h3>{{ $t('Current Appointment') }}</h3>
          <div class="appointment-comparison">
            <div class="old-appointment">
              <div class="comparison-label">{{ $t('Old Schedule') }}</div>
            ...
            <div class="new-appointment">
              <div class="comparison-label">{{ $t('New Schedule') }}</div>
              <div class="appointment-detail">
                <div class="detail-item">
                  <span class="label">{{ $t('Date:') }}</span>
                ...
                <div class="detail-item">
                  <span class="label">{{ $t('Status:') }}</span>
                  <span class="value status-active">{{ $t('Scheduled') }}</span>
                </div>
              </div>
            </div>
          </div>
          <div class="reschedule-note">
            ...
            <div class="note-content">
              <p><strong>{{ $t('Note:') }}</strong> {{ $t('A new appointment will be created and the old appointment will be cancelled.') }}</p>
            </div>
          </div>
        </div>
        ...
        <div class="instructions-card">
          <h3>{{ $t('Important Instructions') }}</h3>
          <ul class="instructions-list">
            <li>{{ $t('Arrive at least 10 minutes before your scheduled time') }}</li>
            <li>{{ $t('Ensure you have a stable internet connection') }}</li>
            <li>{{ $t('Prepare your identification document') }}</li>
            <li>{{ $t('Make sure your webcam and microphone are working') }}</li>
            <li>{{ $t('Clear your workspace of any prohibited materials') }}</li>
            <li>{{ $t('The "Start Exam" button will appear 30 minutes before your appointment') }}</li>
            <li v-if="!isReschedule">{{ $t('You can reschedule up to 2 days before the exam') }}</li>
            <li v-if="isReschedule">{{ $t('Your old time slot will be made available to other students') }}</li>
          </ul>
        </div>
      </div>
      ...
      <div class="step-actions">
        ...
        <button @click="confirmBooking" :disabled="bookingInProgress" class="btn-confirm">
          ...
          <span v-else class="booking-loading">{{ $t('Processing...') }}</span>
    ...
    <div v-if="showSuccessModal" class="modal-overlay" @click.self="closeSuccessModal">
      <div class="modal-content success-modal">
        ...
        <div class="success-actions">
          <button @click="goToExams" class="btn-primary">{{ $t('View My Exams') }}</button>
          ...
          <button v-if="bookedAppointment || existingAppointment"
                  @click="goToAppointment" class="btn-secondary">{{ $t('View Appointment') }}</button>
          <button v-if="!isReschedule" @click="closeSuccessModal" class="btn-secondary">{{ $t('Schedule Another') }}</button>
        </div>
      </div>
    </div>
    ...
    <div v-if="showCancelModal" class="modal-overlay" @click.self="showCancelModal = false">
      <div class="modal-content cancel-modal">
        ...
        <h3>{{ $t('Cancel Appointment') }}</h3>
        <p>{{ $t('Are you sure you want to cancel this exam appointment?') }}</p>
        ...
        <div class="cancel-warning">
          <p><strong>{{ $t('Note:') }}</strong> {{ $t('This action cannot be undone. The time slot will be made available for other students.') }}</p>
        </div>

        <div class="cancel-actions">
          <button @click="showCancelModal = false" class="btn-secondary">{{ $t('Go Back') }}</button>
          <button @click="confirmCancel" :disabled="cancelling" class="btn-danger">
            <span v-if="!cancelling">{{ $t('Yes, Cancel Appointment') }}</span>
            <span v-else class="cancelling-loading">{{ $t('Cancelling...') }}</span>
          </button>
        </div>
      </div>
    </div>
